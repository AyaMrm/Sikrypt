...
pub fn derive_shared_secret(
    ...
    let encoded =
        EncodedPoint::from_bytes(peer_public_key).map_err(|_| EccError::InvalidPublicKey)?;
    let public =
        PublicKey::from_sec1_bytes(encoded.as_bytes()).map_err(|_| EccError::InvalidPublicKey)?;
